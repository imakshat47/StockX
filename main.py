...
if __name__ == '__main__':
    try:
        ...
        csv_data = [
            ...
            ['B', 'TCS', 'NSE', '100', '11.40', '1618148402', '9', '11.38'],
        ...
        for data in csv_data:
            ...
            if(_price == ''):
                if(_rev_obj._max_price == 0):
                    if _obj == '_bid':
                        print_msg(
                            "Market Order can not be Executed. No Sell Order on " + _exch)
                    else:
                        print_msg(
                            "Market Order can not be Executed. No Buy Order on " + _exch)
                    continue
                _rev_obj._execute(_qty)
                print_msg("Market Order Executed @ " + _exch + ". Size = " + str(_qty))
                continue

            # For Stop Loss
            if 0 <= 7 < len(data):
                market_price = input("Enter the Current Market Price: ")
                _sl_price = data[7]
                if data[0] == 'A':
            ...
            if(_check == True):
                print_msg("Limit Order Executed @ " + _exch + ". Size = " + str(_qty))
                continue
